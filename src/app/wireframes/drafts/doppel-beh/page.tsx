...
type Patient = {
  ...
  isTemporaryTransfer?: boolean; // New field to track temporary transfers
  doppelBeh?: boolean; // New field for Doppel Beh modal state
  doppelBehGroup?: string; // New field for calendar grouping operations
  doppelBehEnabled?: boolean; // New field to control if Doppel Beh feature is available for this VO
};
...
type TreatmentEntry = {
  ...
  isDoppelBeh?: boolean; // New field to track if this was a Doppel Beh treatment
  doppelBehGroup?: string; // New field for grouping paired Doppel Beh treatments
};
...
type TreatmentCalendarItem = {
  itemType: 'treatment';
  displayOrder: number;
  patient: Patient;
  treatmentEntry: TreatmentEntry;
  entryId: string; // Unique identifier for this specific treatment entry
};

type DoppelBehGroupCalendarItem = {
  itemType: 'doppelBehGroup';
  displayOrder: number;
  patient: Patient;
  treatmentEntries: TreatmentEntry[]; // Array of 2 treatment entries
  groupId: string; // Unique identifier for this group
};

type CalendarItem = 
  | TreatmentCalendarItem
  | DoppelBehGroupCalendarItem
...
const CalendarView = ({ 
  ...
  const [draggedCalendarItem, setDraggedCalendarItem] = useState<{id: string, type: 'treatment' | 'doppelBehGroup' | 'activity', date: string} | null>(null);
  ...
  // Get individual treatment entries for a specific day
  const getTreatmentEntriesForDay = (date: Date): TreatmentCalendarItem[] => {
    // Format date to DD.MM.YYYY for comparison (to match treatment history format)
    const dateStr = formatDateString(date);
    
    const treatmentEntries: TreatmentCalendarItem[] = [];
    
    treatments.forEach(patient => {
      // Skip patients without treatment history
      if (!patient.treatmentHistory || patient.treatmentHistory.length === 0) {
        return;
      }
      
      // Find all treatment entries that happened on this date
      patient.treatmentHistory.forEach(entry => {
        if (entry.date === dateStr) {
          treatmentEntries.push({
            ...
            entryId: `${patient.id}-${entry.date}-${entry.order || 0}${entry.doppelBehGroup ? '-' + entry.doppelBehGroup : ''}`
          });
        }
      });
    });
  ...
  // Combine treatment entries and activities and sort by order
  const getOrderedItemsForDay = (date: Date): CalendarItem[] => {
    // Get individual treatment entries for this day
    const treatmentEntries = getTreatmentEntriesForDay(date);
    
    // Group Doppel Beh treatments
    const groupedTreatments: CalendarItem[] = [];
    const processedEntries = new Set<string>();
    
    treatmentEntries.forEach(entry => {
      if (processedEntries.has(entry.entryId)) return;
      
      if (entry.treatmentEntry.doppelBehGroup) {
        // Find the pair for this Doppel Beh treatment
        const pairedEntry = treatmentEntries.find(other => 
          other.entryId !== entry.entryId && 
          other.treatmentEntry.doppelBehGroup === entry.treatmentEntry.doppelBehGroup
        );
        
        if (pairedEntry) {
          // Create a group item
          const groupItem: DoppelBehGroupCalendarItem = {
            itemType: 'doppelBehGroup',
            displayOrder: Math.min(entry.displayOrder, pairedEntry.displayOrder),
            patient: entry.patient,
            treatmentEntries: [entry.treatmentEntry, pairedEntry.treatmentEntry].sort((a, b) => (a.order || 0) - (b.order || 0)),
            groupId: `group-${entry.patient.id}-${entry.treatmentEntry.doppelBehGroup}`
          };
          
          groupedTreatments.push(groupItem);
          processedEntries.add(entry.entryId);
          processedEntries.add(pairedEntry.entryId);
        } else {
          // Single Doppel Beh entry (shouldn't happen, but handle gracefully)
          groupedTreatments.push(entry);
          processedEntries.add(entry.entryId);
        }
      } else {
        // Regular treatment
    ...
    return [...groupedTreatments, ...activityItems].sort((a, b) => 
  ...
  const handleCalendarDragStart = (item: CalendarItem, date: Date, e: React.DragEvent) => {
    const itemId = item.itemType === 'treatment' ? item.entryId : 
                   item.itemType === 'doppelBehGroup' ? item.groupId : 
                   item.id.toString();
  ...
  const handleCalendarDrop = (targetItem: CalendarItem, date: Date, e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    
    if (!draggedCalendarItem) {
      return;
    }
    ...
    const targetItemId = targetItem.itemType === 'treatment' ? targetItem.entryId : 
                         targetItem.itemType === 'doppelBehGroup' ? targetItem.groupId :
                         targetItem.id.toString();
    
    if (draggedCalendarItem.id === targetItemId && 
    ...
    const draggedIndex = allItems.findIndex(item => {
      const itemId = item.itemType === 'treatment' ? item.entryId : 
                     item.itemType === 'doppelBehGroup' ? item.groupId :
                     item.id.toString();
      return item.itemType === draggedCalendarItem.type && itemId === draggedCalendarItem.id;
    });
    ...
    const targetIndex = allItems.findIndex(item => {
      const itemId = item.itemType === 'treatment' ? item.entryId : 
                     item.itemType === 'doppelBehGroup' ? item.groupId :
                     item.id.toString();
      return item.itemType === targetItem.itemType && itemId === targetItemId;
    });
    ...
    // Assign new sequential orders accounting for groups taking multiple slots
    let currentOrder = 1;
    
    // Process each item in the new order and update accordingly
    updatedItems.forEach(item => {
      if (item.itemType === 'treatment') {
        const treatmentItem = item as TreatmentCalendarItem;
        const treatmentIndex = updatedTreatments.findIndex(t => t.id === treatmentItem.patient.id);
        if (treatmentIndex !== -1) {
          const treatment = updatedTreatments[treatmentIndex];
          if (treatment.treatmentHistory) {
            const entryIndex = treatment.treatmentHistory.findIndex(entry => 
              entry.date === dateStr && 
              entry.order === treatmentItem.treatmentEntry.order &&
              entry.doppelBehGroup === treatmentItem.treatmentEntry.doppelBehGroup
            );
            if (entryIndex !== -1) {
        ...
        currentOrder++; // Single treatment takes 1 slot
      } else if (item.itemType === 'doppelBehGroup') {
        const groupItem = item as DoppelBehGroupCalendarItem;
        const treatmentIndex = updatedTreatments.findIndex(t => t.id === groupItem.patient.id);
        if (treatmentIndex !== -1) {
          const treatment = updatedTreatments[treatmentIndex];
          if (treatment.treatmentHistory) {
            // Update both treatment entries in the group with consecutive orders
            groupItem.treatmentEntries.forEach((groupEntry, idx) => {
              const entryIndex = treatment.treatmentHistory!.findIndex(entry => 
                entry.date === dateStr && 
                entry.order === groupEntry.order &&
                entry.doppelBehGroup === groupEntry.doppelBehGroup
              );
              if (entryIndex !== -1) {
                const updatedHistory = [...treatment.treatmentHistory!];
                updatedHistory[entryIndex] = {
                  ...updatedHistory[entryIndex],
                  order: currentOrder + idx // Assign consecutive orders within group
                };
                updatedTreatments[treatmentIndex] = {
                  ...updatedTreatments[treatmentIndex],
                  treatmentHistory: updatedHistory
        ...
        currentOrder += 2; // Doppel Beh group takes 2 slots
      } else if (item.itemType === 'activity') {
        const activityItem = item as DayActivity & { itemType: 'activity', displayOrder: number };
        const activityIndex = updatedActivities.findIndex(a => a.id === activityItem.id && a.date === dateStr);
        if (activityIndex !== -1) {
        ...
        currentOrder++; // Single activity takes 1 slot
      }
    });
  ...
  const renderCalendarItem = (item: CalendarItem, date: Date, idx: number) => {
    ...
    const isBeingDragged = draggedCalendarItem && 
                          draggedCalendarItem.id === itemId && 
                          draggedCalendarItem.type === item.itemType &&
                          draggedCalendarItem.date === formatDateString(date);
                              
    if (item.itemType === 'treatment') {
      const treatmentItem = item as TreatmentCalendarItem;
      const treatment = treatmentItem.patient;
      const historyEntry = treatmentItem.treatmentEntry;
      ...
      return (
        <div 
          key={`treatment-${treatment.id}-${historyEntry.order}-${idx}`}
          onClick={() => onEditItem({ type: 'treatment', id: treatment.id, date: formatDateString(date) })}
          ...
          onDrop={(e) => handleCalendarDrop(item, date, e)}
      ...
      // This is a Doppel Beh group
      const groupItem = item as DoppelBehGroupCalendarItem;
      const treatment = groupItem.patient;
      
      return (
        <div 
          key={`doppel-beh-group-${groupItem.groupId}-${idx}`}
          className={`mb-2 p-3 border-2 border-blue-300 bg-blue-50 rounded-lg shadow-sm cursor-pointer ${
          ...
          onDragEnd={handleCalendarDragEnd}
        >
          <div className="text-xs font-semibold text-blue-700 mb-2">Doppel Beh Treatment</div>
          
          {groupItem.treatmentEntries.map((entry, entryIdx) => {
            const isRejectedWithSignature = entry.session?.startsWith('Rejected-') && 
                                           entry.session !== 'Rejected-NoCount' && 
                                           entry.signatureObtained;
                                           
            const isRejectedWithoutSignature = entry.session === 'Rejected-NoCount' || 
                                              (entry.session?.startsWith('Rejected-') && !entry.signatureObtained);
            ...
            return (
              <div 
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onEditItem({ type: 'treatment', id: treatment.id, date: formatDateString(date) });
                }}
                className={`mb-1 p-2 border rounded-md shadow-sm ${
                  isRejectedWithSignature 
                ...
                <div className="flex justify-between items-start mb-1">
                  <div>
                    ...
                    <div className={`text-xs ${
                      ...
                      {treatment.prescription} | {entry.session}
                      {isRejectedWithSignature && (
                        <span className="ml-1 px-1.5 py-0.5 text-xs font-semibold bg-orange-200 text-orange-800 rounded-full">
                          Rejected (✓)
                        </span>
...
export default function TreatmentDocumentationV2Wireframe() {
  ...
  const [transferType, setTransferType] = useState<'temporary' | 'permanent' | null>(null);
  ...
  // Therapist list
  const therapists = ['Sarah Miller', 'John Schmidt'];
  ...
  const transferTherapists = ['Sarah Miller', 'John Schmidt', 'Lisa Thompson'];
  ...
  // Filtered patients based on selected therapist
  const patients = allPatients.filter(patient => patient.therapeut === selectedTherapistFilter);
  ...
  // Function to update Doppel Beh status
  const updatePatientDoppelBeh = (id: number, doppelBeh: boolean) => {
    setModalPatients(modalPatients.map(patient =>
      patient.id === id && !('isBreak' in patient) ? { 
        ...patient, 
        doppelBeh,
        // Clear rejection when Doppel Beh is selected
        rejectedTreatment: doppelBeh ? false : patient.rejectedTreatment
      } : patient
    ) as ModalItem[]);
  };
    ...
    if (editingItem) {
      ...
      const updatedPatients = patients.map(patient => {
        ...
        if (modalPatient) {
          ...
          // Calculate increment amount: 2 for Doppel Beh, 1 for regular
          const incrementAmount = modalPatient.doppelBeh ? 2 : 1;
          
          // Only increment completedTreatments if we should count this treatment
          const newCompletedTreatments = shouldIncrementCount 
            ? Math.min(patient.completedTreatments + incrementAmount, patient.totalTreatments)
          ...
          return {
            ...
            treatmentHistory: [
              ...
              // Create treatment history entries - either 1 regular or 2 for Doppel Beh
              ...(modalPatient.doppelBeh ? [
                // First Doppel Beh entry
                {
                  date: treatmentDate,
                  notes: modalPatient.notes,
                  session: `${Math.min(patient.completedTreatments + 1, patient.totalTreatments)}/${patient.totalTreatments} (Doppel Beh 1)`,
                  order: modalPatient.order || 0,
                  isDoppelBeh: true,
                  doppelBehGroup: `${patient.id}-${treatmentDate}-doppel`
                },
                // Second Doppel Beh entry
                {
                  date: treatmentDate,
                  notes: modalPatient.notes,
                  session: `${Math.min(patient.completedTreatments + 2, patient.totalTreatments)}/${patient.totalTreatments} (Doppel Beh 2)`,
                  order: (modalPatient.order || 0) + 0.1, // Slightly different order to ensure proper sorting
                  isDoppelBeh: true,
                  doppelBehGroup: `${patient.id}-${treatmentDate}-doppel`
                }
              ] : [
        ...
        {!collapsedCards.includes(patient.id) && (
          <div className="p-4 border-t border-gray-200">
            ...
            */}
            ...
            {/* Doppel Beh Checkbox - Only show if doppelBehEnabled is true */}
            {patient.doppelBehEnabled && (
              <div className="mt-4">
                <label className="flex items-center text-sm text-gray-700">
                  <input 
                    type="checkbox"
                    checked={!!patient.doppelBeh}
                    onChange={(e) => updatePatientDoppelBeh(patient.id, e.target.checked)}
                    disabled={patient.totalTreatments - patient.completedTreatments <= 1}
            ...
            {/* Patient Rejected Treatment Checkbox - Only show when Doppel Beh is not checked */}
            {!patient.doppelBeh && (
              <div className="mt-4">
                <label className="flex items-center text-sm text-gray-700">
                  <input 
  ...
  const confirmTransfer = async () => {
    ...
    if (!transferType) {
      addToast('Please select a transfer type.', 'error');
      return;
    }
      ...
      const updatedPatients = allPatients.map(p => {
        if (selectedPatientIds.includes(p.id)) {
          try {
            ...
            successfulTransfers.push(p);
            return { 
              ...p, 
              therapeut: selectedTherapist, // Update to the new therapist
              isTemporaryTransfer: transferType === 'temporary' // Set temporary transfer flag
            };
          } catch (error) {
  ...
  return (
    <WireframeLayout title="Doppel Beh" username="User Therapist" userInitials="UT" showSidebar={false}>
        ...
        {(activeTab === 'active-patients' || activeTab === 'inactive-patients') && (
          <div className="bg-white rounded-lg shadow-sm mb-4">
            ...
            <div className="grid grid-cols-11 gap-4 py-3 px-4 bg-gray-100 text-gray-700 font-medium text-sm">
              ...
              <div className="col-span-1">Doppel. Beh</div> {/* New Doppel Beh Column */}
              <div className="col-span-1">Beh Status (#/#)</div>
            ...
            {(activeTab === 'active-patients' ? patients : inactivePatients).map((patient) => (
              <div 
                key={patient.id} 
                className={`grid grid-cols-11 gap-4 py-3 px-4 border-t border-gray-200 items-center ${
              ...
              >
                ...
                <div className="col-span-1 flex flex-col">
                  <span>{patient.therapeut}</span>
                  {patient.isTemporaryTransfer && <TemporaryTransferBadge />}
                </div>
                <div className="col-span-1">
                ...
                <div className="col-span-1">
                  ...
                  <span className="font-medium">
                    {patient.doppelBehEnabled ? 'Ja' : 'Nein'}
                  </span>
                </div>
                <div className="col-span-1">
                        ...
                        >
                          ...
                          <div className={`col-span-5 ${
                            ...
                            {isRejected ? (
                              ...
                              entry.isDoppelBeh ? (
                                <>
                                  <strong>Doppel Beh</strong> : {displayNotes}
                                </>
                              ) : (
                                displayNotes
                              )
      ...
      {isTransferModalOpen && selectedPatientsForTransfer.length > 0 && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-auto max-h-[90vh] flex flex-col">
            {/* Modal Header */}
            <div className="py-4 px-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-800">
                Transfer Patient{selectedPatientsForTransfer.length > 1 ? 's' : ''} ({selectedPatientsForTransfer.length})
              </h2>
            </div>
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              ...
              <div className="mb-6">
                ...
                <div className="max-w-md">
                  <select
                    value={transferType || ''}
                    onChange={(e) => setTransferType(e.target.value as 'temporary' | 'permanent')}
                  ...
                  <div className="mt-3 p-3 rounded-md bg-gray-50 border border-gray-200">
                    <p className="text-sm text-gray-700">
                      {transferType === 'temporary' ? (
                        <>
                          <strong>Temporary Transfer:</strong> The original therapist retains ownership of the VO in TheOrg. This is typically used for short-term coverage.
                        </>
                      ) : (
                        <>
              ...
              {/* Patient Table */}
              <div className={`mb-6 ${!transferType ? 'opacity-50 pointer-events-none' : ''}`}>
                <h3 className="text-lg font-medium text-gray-900 mb-3">Selected Patients</h3>
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  {/* Table Header */}
              ...
              <div className={`mb-4 ${!transferType ? 'opacity-50 pointer-events-none' : ''}`}>
                <label className="block text-lg font-medium text-gray-900 mb-3">
                ...
                <div className="max-w-md">
                  ...
                  <div className="relative" id="transfer-therapist-dropdown">
                    <div className="relative">
                      <input
                        type="text"
                        value={transferSearchTerm}
                        onChange={handleTransferInputChange}
                        onFocus={handleTransferInputFocus}
                      ...
                      />
                      <svg
                        className={`absolute right-2 top-1/2 transform -translate-y-1/2 w-4 h-4 transition-transform pointer-events-none ${isTransferDropdownOpen ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    {isTransferDropdownOpen && !isTransferring && (
                      <div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg z-50">
                        <div className="max-h-48 overflow-y-auto">
                          {filteredTransferTherapists.length > 0 ? (
                            // Show filtered results, excluding current therapists of selected patients
                            filteredTransferTherapists
                              .filter(therapist => !selectedPatientsForTransfer.some(patient => patient.therapeut === therapist))
                              .map((therapist) => (
                                <div
                                  key={therapist}
                                  onClick={() => handleTransferTherapistSelect(therapist)}
                                  className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-100 ${
                                    selectedTherapist === therapist ? 'bg-blue-50 text-blue-600' : ''
                                  }`}
                                >
                                  {therapist}
                                </div>
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 mt-1">
                      All {selectedPatientsForTransfer.length} selected patient{selectedPatientsForTransfer.length > 1 ? 's' : ''} will be transferred to the chosen therapist. Individual audit logs will be created for each transfer.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3 p-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
              ...
              <button 
                ...
                disabled={!selectedTherapist || !transferType || isTransferring}
                className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 ${
                  selectedTherapist && transferType && !isTransferring
              ...
              >
                ...
                {isTransferring ? 'Transferring...' : 
                 transferType ? `Confirm ${transferType === 'temporary' ? 'Temporary' : 'Permanent'} Transfer` : 'Confirm Transfer'}
              </button>
            </div>
          </div>
        </div>
