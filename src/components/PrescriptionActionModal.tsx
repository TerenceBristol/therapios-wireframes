...
interface PrescriptionActionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const PrescriptionActionModal: React.FC<PrescriptionActionModalProps> = ({ isOpen, onClose, onConfirm, title, patientData }) => {
  ...
  const handleConfirmClick = () => {
    ...
    if (selectedReason === 'other') {
      finalReason += otherReason ? `: ${otherReason}` : ': Other';
    }
    console.log('Action Confirmation Reason:', finalReason);
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Reason for non-renewal:</label>
